  ...
  <div class="habits-card">
    ...
    <div class="habits-list">
      <div v-for="(habit, index) in habits" :key="index" class="habit-item fade-in">
        <div class="habit-check" @click="$emit('toggle-habit', index)">
...
export default {
  ...
  computed: {
    progreso() {
      ...
      if (total === 0) return 0
      return Math.round((this.habits.filter(h => h.hecho).length / total) * 100)
    }
  },
  emits: ['toggle-habit']
}
...
.habit-name { display: block; font-size: 14px; font-weight: 600; color: var(--text-primary); }
.habit-name.hecho { text-decoration: line-through; color: var(--text-muted); }
.habit-racha { font-size: 11px; color: var(--text-muted); }
